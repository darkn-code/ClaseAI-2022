...
def abrirPuerto(arduino,puerto,codigo):
    try:
        arduino.open()
        ...
        hou = str(ct.time())
        luv = datos.split(',')[2]
        newData = tem+","+hum+","+codigo+","+dat+","+hou+","+luv
        newData = newData[:-1]
...
def main():
    noserie = revisarArchivo(NO_SERIE)
    if noserie != '':
        cod = noserie
    else:
        cod = codeGen(10,False)
        with open(NO_SERIE,'w',encoding='utf-8') as f:
            f.write(cod)
            f.close()
    ...
    arduino.port=puerto
    if puerto != '':
        abrirPuerto(arduino,puerto,cod)
    else:
